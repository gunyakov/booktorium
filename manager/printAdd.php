<?php
/***************************************************************************************************
 * Скрипт: добавление издательств в БД
 ***************************************************************************************************
 * Version 		  : 1.0 stable
...
//---------------------------------------------------------------------------------------------------------------
//Если пользователю разрешено добавлять издательства
//---------------------------------------------------------------------------------------------------------------
if ($objOptions -> getOption("PrintAdd") == "yes") {
	//Если приняты данные формы
	if (readValue("action") == "addPrint") {
		//Читаем данные формы
		$city = readvalue("city");
		$name = readValue("name");
		//Если принятые данные соответсвуют требуемому формату
		if (checkValue($city, "simbols") && checkValue($name, "simbols")) {
			//Пробуем выбрать запись из БД с такими же данными
			$data = $objDB -> select("SELECT * FROM printList WHERE name LIKE '" . $name . "' AND city LIKE '" . $city . "';");
			//Если записи в БД не сужествует
			if (!$data) {
				//Если удалось добавить запись в БД
				if ($objDB -> insert("printList", Array("name" => $name, "city" => $city, "userID" => $objSession -> getUserID(), "datePut" => "NOW()"))) {
	...
	//Если не приняты данные формы
		...
		$objTheme -> define(Array("MAIN_CONTENT" => "addPrintForm.tpl"));
	}
} 
//---------------------------------------------------------------------------------------------------------------
