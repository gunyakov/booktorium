<?php
//-------------------------------------------------------------------------------------------------------------------------
// Модуль: конвертация страниц djvu книги в изображения и распознавание страниц
//-------------------------------------------------------------------------------------------------------------------------
// Version 		    : 2.0 b
...
//-------------------------------------------------------------------------------------------
    ...
    if (!is_file($tiffImageName)) {
        //Получаем TIFF из DJVU
        exec(ddjvu . " -format=tiff -aspect=yes -page=" . $currentPage . " " . FILES_STORAGE . $currentFile['storage'] . "/" . $currentFile['fileName'] . " " . $tiffImageName);
    }
        ...
        switch (readValue("mode")) {
            case "text" :
                ...
                if (!is_file($imagePath . $imageName . ".txt")) {
                    ...
                    if (!Limits("ReadText")) {
                        //Распознаем текст
                        //echo(cuneiform . " -f text -l ruseng -o " . $imagePath . $imageName . ".txt " . $tiffImageName);
